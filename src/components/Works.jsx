...
const ProjectCard = ({
  ...
  // 手机端直接显示 Tilt，不使用 motion
  if (mobile) return CardContent;

  return <motion.div variants={fadeIn("up", "spring", index * 0.3, 0.75)}>{CardContent}</motion.div>;
};

const Works = () => {
  const mobile = isMobile();

  const SectionHeader = mobile ? (
    <div>
  ...
  const SectionDescription = mobile ? (
  ...
  return (
    <>
      {SectionHeader}
      <div className="w-full flex">{SectionDescription}</div>
